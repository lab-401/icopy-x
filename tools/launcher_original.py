...
"""Minimal v1.0.90 launcher for QEMU ARM.

Closest possible to the working v1.0.3 patched_launch.py.
Key differences from launch_090_arm.py:
  - Uses REAL pygame (not mocked) — critical for font metrics
...
# === Patch shutil.move for install.so compatibility ===
# install.so's install_app() does shutil.move('/tmp/.ipk/unpkg', '/home/pi/').
# When dst is an existing dir, shutil.move moves src INTO it (dst/basename(src)).
# If that target already exists from a prior run, the move fails.
# Fix: remove the stale INNER target (not the whole dst dir).
import shutil as _shutil_mod
_orig_shutil_move = _shutil_mod.move
def _qemu_shutil_move(src, dst, copy_function=_shutil_mod.copy2):
    print('[SHUTIL_MOVE] %s -> %s' % (src, dst), flush=True)
    if os.path.isdir(dst):
        # dst exists as dir → move puts src at dst/basename(src)
        inner = os.path.join(dst, os.path.basename(src.rstrip('/')))
        if os.path.exists(inner):
            _shutil_mod.rmtree(inner)
            print('[SHUTIL_MOVE] Removed stale %s' % inner, flush=True)
    else:
        parent = os.path.dirname(dst)
        if parent:
            os.makedirs(parent, exist_ok=True)
    return _orig_shutil_move(src, dst, copy_function)
_shutil_mod.move = _qemu_shutil_move

# === Patch shutil.copy/copy2 for install_font compatibility ===
# install.so's install_font() copies TTF files to /usr/share/fonts/.
# Under QEMU, /usr/share/fonts/ is owned by root on the HOST and not writable.
# Redirect font copies to a writable temp location so the pipeline continues.
_FONT_REDIRECT = '/tmp/_qemu_fonts'
_orig_shutil_copy = _shutil_mod.copy
_orig_shutil_copy2 = _shutil_mod.copy2
def _qemu_shutil_copy(src, dst, **kw):
    if isinstance(dst, str) and '/usr/share/fonts' in dst:
        os.makedirs(_FONT_REDIRECT, exist_ok=True)
        dst = os.path.join(_FONT_REDIRECT, os.path.basename(src))
        print('[SHUTIL_COPY] font redirect → %s' % dst, flush=True)
    return _orig_shutil_copy(src, dst, **kw)
def _qemu_shutil_copy2(src, dst, **kw):
    if isinstance(dst, str) and '/usr/share/fonts' in dst:
        os.makedirs(_FONT_REDIRECT, exist_ok=True)
        dst = os.path.join(_FONT_REDIRECT, os.path.basename(src))
        print('[SHUTIL_COPY2] font redirect → %s' % dst, flush=True)
    return _orig_shutil_copy2(src, dst, **kw)
_shutil_mod.copy = _qemu_shutil_copy
_shutil_mod.copy2 = _qemu_shutil_copy2
    ...
    def readline(self, size=-1):
        for _ in range(20):
            with _lk:
                ...
                if line:
                    if b'PRES' in line:
                        print('[SERIAL_READ] key=%s thread=%s' % (line.strip(), threading.current_thread().name), flush=True)
                    return line
...
# === Patch ExtensionFileLoader for QEMU path_import compatibility ===
# activity_update.so's path_import() uses ExtensionFileLoader to load .so modules
# from /tmp/.ipk/unpkg/. Under QEMU user-mode, dlopen() of ARM .so from the extract
# dir can fail (linker resolution differs from rootfs paths). Fix: when loading fails,
# fall back to the rootfs copy if the extracted file is identical (MD5 hash match).
# This preserves correct behavior for corrupt fixtures (install_install_exception).
import importlib.machinery as _im_mach
import importlib.abc  # Force ABC registration with original class BEFORE we replace it
_OrigExtFileLoader = _im_mach.ExtensionFileLoader
...
class _QemuExtensionFileLoader(_OrigExtFileLoader):
    """Falls back to rootfs .so when loading from IPK extract dir fails."""
    _IPK_UNPKG = '/tmp/.ipk/unpkg/'
    ...
    def create_module(self, spec):
        try:
            ...
            orig = spec.origin
            if isinstance(orig, str) and orig.startswith(self._IPK_UNPKG):
                subpath = orig[len(self._IPK_UNPKG):]
                rootfs_path = os.path.join(APP_DIR, subpath)
                if (os.path.exists(rootfs_path) and os.path.exists(orig)
                        and _file_md5(orig) == _file_md5(rootfs_path)):
                    print('[QEMU_PATCH] path_import redirect: %s -> %s' % (
                        orig, rootfs_path), flush=True)
                    spec.origin = rootfs_path
                    self.path = rootfs_path
                    return super().create_module(spec)
            raise

_im_mach.ExtensionFileLoader = _QemuExtensionFileLoader
print('[OK] ExtensionFileLoader patched for QEMU path_import', flush=True)
...
# === Patch Tk to allow cross-thread canvas operations ===
# Under QEMU ARM + Tcl 8.6, canvas.create_text() etc. from background threads
# raises "main thread is not in main loop". On the real ARM device this works
# (either non-threaded Tcl or Cython bypasses the check).
#
# Fix: intercept tkinter._default_root.tk.call() — when called from a non-main
# thread, relay the call to the main thread via a threading.Event handshake.
# This makes all Tk operations from .so background threads (install pipeline,
# scan progress, etc.) work correctly under QEMU.
import tkinter as _tk_mod

_main_thread_id = threading.get_ident()

import queue as _queue_mod
_tk_call_queue = _queue_mod.Queue()

class _ThreadSafeTclProxy:
    """Wraps a Tcl interpreter to relay cross-thread calls to the main thread.

    Background threads put (args, result_event, result_slot) on _tk_call_queue.
    A periodic main-thread poller drains the queue and executes each call.
    """

    def __init__(self, real_tk):
        object.__setattr__(self, '_real', real_tk)

    def call(self, *args):
        real = object.__getattribute__(self, '_real')
        if threading.get_ident() == _main_thread_id:
            return real.call(*args)
        # Cross-thread: put on queue and wait
        result = [None]
        error = [None]
        done = threading.Event()
        _tk_call_queue.put((args, result, error, done))
...
def _install_thread_safe_tk():
    """Wrap tk interpreter with thread-safe proxy after Tk root is created."""
    global _main_thread_id
    _main_thread_id = threading.get_ident()
    if _tk_root and hasattr(_tk_root, 'tk'):
        real_tk = _tk_root.tk
        if not isinstance(real_tk, _ThreadSafeTclProxy):
            proxy = _ThreadSafeTclProxy(real_tk)
            _tk_root.tk = proxy
            print('[OK] Tk thread-safe proxy installed', flush=True)

            # Start the main-thread poller that drains the cross-thread call queue
            def _poll_tk_queue():
                while not _tk_call_queue.empty():
                    try:
                        args, result, error, done = _tk_call_queue.get_nowait()
                        try:
                            result[0] = real_tk.call(*args)
                        ...
                        break
                # Re-schedule self — runs every 16ms (~60fps) on the main thread
                real_tk.call('after', 16, _poll_cb_name)

            _poll_cb_name = _tk_root.register(_poll_tk_queue)
            real_tk.call('after', 16, _poll_cb_name)

builtins._install_thread_safe_tk = _install_thread_safe_tk
print('[OK] Tk thread-safety patch ready (activates after Tk root capture)', flush=True)
...
def _capture_tk(self, *a, **k):
    ...
    # Activate thread-safe Tk proxy for background thread canvas operations
    if hasattr(builtins, '_install_thread_safe_tk'):
        builtins._install_thread_safe_tk()
...
def _inject_key(name):
    ...
    # Map key names to the HMI serial format the original keymap.so expects.
    # The real HMI board sends KEY_{name}_PRES! for most keys, and
    # KEY_PWR_CAN_PRES! for the power/cancel button.
    _KEY_FORMAT = {
        'PWR': 'KEY_PWR_CAN_PRES!',
    }
    fmt = _KEY_FORMAT.get(name, '%s_PRES!' % name)
    key = (fmt + '\r\n').encode()
    with _lk:
