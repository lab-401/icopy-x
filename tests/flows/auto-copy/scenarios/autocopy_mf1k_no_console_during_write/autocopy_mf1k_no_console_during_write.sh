#!/bin/bash
# Negative test: RIGHT during Auto-Copy WRITE phase should NOT open console
PROJECT="${PROJECT:-/home/qx/icopy-x-reimpl}"
SCENARIO="autocopy_mf1k_no_console_during_write"
FLOW="auto-copy"
PM3_DELAY=0.5
BOOT_TIMEOUT=600
...
# Source auto-copy common for wait_for_ui_trigger
source "${PROJECT}/tests/flows/auto-copy/includes/auto_copy_common.sh"

check_env
clean_scenario

RAW_DIR="/tmp/raw_ac_console_write_${SCENARIO}"
mkdir -p "${RAW_DIR}"
...
# Enter Auto-Copy (GOTO:0) — scan+read starts automatically
send_key "GOTO:0"

# Wait for scan+read to complete (M2 shows "Write")
if ! wait_for_ui_trigger "M2:Write" 240 "${RAW_DIR}" FRAME; then
    dedup_screenshots "${RAW_DIR}" "${SCREENSHOTS_DIR}"
    report_fail "Read phase did not complete (M2:Write not seen, ${DEDUP_COUNT:-0} states)"
    cleanup_qemu
    rm -rf "${RAW_DIR}"
    exit 1
fi

# Press M2 to start the write phase
send_key "M2"
sleep 3

# Now we should be in the write phase — capture baseline
FRAME=$((FRAME+1))
...
# Send RIGHT — should have NO effect during write
send_key "RIGHT"
sleep 1
...
# Verdict: PASS if RIGHT did not change the screen during write
if [ "${BASELINE_HASH}" = "${AFTER_HASH}" ]; then
    ...
    report_pass "RIGHT had no effect during AC write phase (${DEDUP_COUNT} states)"
else
    report_fail "Screen changed after RIGHT during AC write (hash mismatch, ${DEDUP_COUNT} states)"
fi
