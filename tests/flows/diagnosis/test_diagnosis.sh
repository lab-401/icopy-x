#!/bin/bash
# Run ALL diagnosis scenarios sequentially.
# Each scenario gets its own QEMU boot.
# Output: _results/diagnosis/scenarios/<name>/{screenshots/,logs/,result.txt}
# Summary: _results/diagnosis/scenario_summary.txt

set +e
PROJECT="${PROJECT:-/home/qx/icopy-x-reimpl}"
RESULTS_DIR="${RESULTS_DIR:-${PROJECT}/tests/flows/_results/${TEST_TARGET:-original}}"
DIAG_RESULTS="${RESULTS_DIR}/diagnosis"
SUMMARY="${DIAG_RESULTS}/scenario_summary.txt"
...
echo "========================================"
echo "  DIAGNOSIS FLOW — ALL SCENARIOS"
echo "  Output: ${DIAG_RESULTS}/"
echo "========================================"

for script in "${PROJECT}/tests/flows/diagnosis/scenarios"/*/diag_*.sh; do
    [ -f "$script" ] || continue
    TOTAL=$((TOTAL + 1))
    ...
    # Check result
    result_file="${DIAG_RESULTS}/scenarios/${scenario_name}/result.txt"
