#!/bin/bash
# Run ALL time_settings scenarios in parallel using worker pools.
set +e

PROJECT="${PROJECT:-$(cd "$(dirname "$0")/../../.." && pwd)}"
RESULTS_DIR="${RESULTS_DIR:-${PROJECT}/tests/flows/_results/${TEST_TARGET:-original}}"
TS_RESULTS="${RESULTS_DIR}/time_settings"
SUMMARY="${TS_RESULTS}/scenario_summary.txt"
...
case "$CLEAN_MODE" in
    ...
    flow) rm -rf "${TS_RESULTS}" ;;
    none) ;;
esac
mkdir -p "${TS_RESULTS}"

SCENARIO_BASE="${PROJECT}/tests/flows/time_settings/scenarios"
SCRIPTS=()
for scenario_dir in "${SCENARIO_BASE}"/time_*/; do
    name="$(basename "$scenario_dir")"
    script="${scenario_dir}/${name}.sh"
    [ -f "$script" ] && SCRIPTS+=("$script")
done

echo "=== Time Settings Flow: ${#SCRIPTS[@]} scenarios ==="
...
for script in "${SCRIPTS[@]}"; do
    scenario_name="$(basename "$(dirname "$script")")"
    result_file="${TS_RESULTS}/scenarios/${scenario_name}/result.txt"
...
{
    echo "========================================"
    echo "  TIME SETTINGS FLOW TEST SUMMARY"
    echo "========================================"
    echo "  Total:   ${TOTAL}"
    echo "  PASS:    ${PASS}"
    echo "  FAIL:    ${FAIL}"
