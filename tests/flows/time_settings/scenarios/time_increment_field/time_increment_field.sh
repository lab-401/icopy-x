#!/bin/bash
# Time Settings: UP increments the current field directly (no OK needed)
# Ground truth: QEMU explore_simple_up — UP on year: 2026→2027 with 2s sleeps
PROJECT="${PROJECT:-/home/qx/icopy-x-reimpl}"
SCENARIO="time_increment_field"
FLOW="time_settings"
...
# UP to increment year (no OK needed)
send_key "UP"
sleep 2
frame_idx=$((frame_idx + 1))
capture_frame_with_state "${raw_dir}" "${frame_idx}"
...
if [ -n "${before}" ] && [ -n "${after}" ] && [ "${before}" != "${after}" ]; then
    ...
    report_pass "UP increments year: ${before} -> ${after} (${DEDUP_COUNT} states)"
else
    report_fail "Year did not change after UP (before=${before}, after=${after})"
fi
