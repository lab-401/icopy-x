#!/bin/bash
# Run ALL backlight scenarios sequentially.
# Each scenario gets its own QEMU boot.
# Output: _results/backlight/scenarios/<name>/{screenshots/,logs/,result.txt}
# Summary: _results/backlight/scenario_summary.txt

set +e
PROJECT="${PROJECT:-/home/qx/icopy-x-reimpl}"
RESULTS_DIR="${RESULTS_DIR:-${PROJECT}/tests/flows/_results/${TEST_TARGET:-original}}"
BACKLIGHT_RESULTS="${RESULTS_DIR}/backlight"
SUMMARY="${BACKLIGHT_RESULTS}/scenario_summary.txt"

mkdir -p "${BACKLIGHT_RESULTS}"
> "${SUMMARY}"
...
echo "========================================"
echo "  BACKLIGHT FLOW — ALL SCENARIOS"
echo "  Output: ${BACKLIGHT_RESULTS}/"
echo "========================================"

for script in "${PROJECT}/tests/flows/backlight/scenarios"/*/backlight_*.sh; do
    [ -f "$script" ] || continue
    TOTAL=$((TOTAL + 1))
    ...
    # Check result
    result_file="${BACKLIGHT_RESULTS}/scenarios/${scenario_name}/result.txt"
