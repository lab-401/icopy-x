#!/bin/bash
# Run ALL scan scenarios in parallel using worker pools.
# Each scenario gets its own Xvfb display — fully isolated.
#
# Usage:
#   bash tests/flows/scan/test_scans_parallel.sh [JOBS]
#   bash tests/flows/scan/test_scans_parallel.sh --clean-flow-only [JOBS]
...
SCRIPTS=()
for scenario_dir in "${PROJECT}/tests/flows/scan/scenarios"/scan_*/; do
    name="$(basename "$scenario_dir")"
    script="${scenario_dir}/${name}.sh"
    [ -f "$script" ] && SCRIPTS+=("$script")
done

echo "=== Scan Flow: ${#SCRIPTS[@]} scenarios ==="
...
{
    echo "========================================"
    echo "  SCAN FLOW TEST SUMMARY"
    echo "========================================"
    echo "TOTAL: ${TOTAL}  PASS: ${PASS}  FAIL: ${FAIL}"
    echo "Duration: ${ELAPSED}s (${JOBS} workers)"
    echo "========================================"
} | tee -a "${SUMMARY}"

cp "${SUMMARY}" "${SCAN_RESULTS}/scan_summary.txt" 2>/dev/null
