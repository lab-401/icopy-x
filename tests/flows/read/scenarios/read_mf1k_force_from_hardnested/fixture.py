# MF Classic 1K: Force Read after nested "not vulnerable"
# Flow: fchk 0 keys → darkside key → nested fails → Warning → Force Read → partial data
SCENARIO_RESPONSES = {
...
[-] isOk:00
'''),
    # Sequential fchk: first call (initial) returns 0 keys,
    # second call (Force Read) returns the darkside-recovered key for sector 0.
    # On real device, darkside writes found key to /tmp/.keys/mf_tmp_keys between calls.
    'hf mf fchk': [
        (0, '''[usb] pm3 --> hf mf fchk 1
...
[+] ( 0:Failed / 1:Success)
'''),
        # Second fchk (Force Read phase): darkside key now in key file
        (0, '''[usb] pm3 --> hf mf fchk 1
[+] found keys:
[+] |-----|----------------|---|----------------|---|
[+] | Sec | key A          |res| key B          |res|
