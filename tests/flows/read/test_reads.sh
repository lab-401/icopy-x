#!/bin/bash
# Run ALL read scenarios sequentially.
# Each scenario gets its own QEMU boot.
# Output: flow_results/read/scenarios/<name>/{screenshots/,logs/,result.txt}
# Summary: flow_results/read/scenario_summary.txt
...
# Also write read_summary.txt at the flow level
cp "${SUMMARY}" "${READ_RESULTS}/read_summary.txt"
