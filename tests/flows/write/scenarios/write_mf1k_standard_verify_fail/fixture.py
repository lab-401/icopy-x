...
# Write succeeds but verification detects card removed/changed.
# Sequential hf 14a info: first 2 calls succeed (read+write scan),
# 3rd+ call returns -1 (card removed during verify).
SCENARIO_RESPONSES = {
...
'''),
    'hf 14a info': [
        # Call 1: read phase scan — success
...
[+] Prng detection: weak
'''),
        # Call 2: write phase scan — success
        (0, '''[usb] pm3 --> hf 14a info
...
[+] Prng detection: weak
'''),
        # Call 3: post-write check — still the same UID (write success toast)
        (0, '''[usb] pm3 --> hf 14a info
...
[+] Prng detection: weak
'''),
        # Call 4+: verify — card removed (tag not found)
        (-1, ''),
    ],
