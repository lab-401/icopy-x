...
# T55XX block write FAILURE flow:
#   READ: detect(original) x2 → read blocks → dump → SUCCESS (reach WarningWrite)
#   WRITE: wipe → detect(wiped) x2 → write block → FAIL (-1)
#
# lf t55xx detect uses SEQUENTIAL responses:
#   calls 1-2: original config (READ phase — must succeed to reach WarningWrite)
#   calls 3+:  wiped config (after wipe)
...
'''),
    # Sequential: first 2 reads (READ phase) return normal data for dump creation,
    # remaining reads (WRITE verify phase) return mismatched data → "Write failed!"
    'lf t55xx read b': [
        (0, '''[+] Reading Page 0:
[+] blk | hex data | binary                           | ascii
[+] ----+----------+----------------------------------+-------
...
[+] blk | hex data | binary                           | ascii
[+] ----+----------+----------------------------------+-------
[+]  01 | DEADBEEF | 11011110101011011011111011101111 | ....
'''),
    ],
...
'''),
    ...
    # restore succeeds, but verify reads back wrong data → fail
    'lf t55xx restore': (0, '''[usb] pm3 --> lf t55xx restore
[+] restoring 12 blocks from file
'''),
