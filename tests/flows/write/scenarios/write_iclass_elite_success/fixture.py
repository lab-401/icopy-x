...
# iCLASS Elite (type 18): all legacy keys fail during scan → chk finds elite key
# Write phase: wrbl succeeds for all data blocks + password
#
# Dict order matters — mock matches FIRST substring hit.
# More specific patterns MUST come before less specific ones.
SCENARIO_RESPONSES = {
    # --- Scan phase: type detection ---
    # All legacy key rdbl calls target block 01 — must all fail for elite
    'hf iclass rdbl b 01': (0, '''[-] Error reading block
'''),
    # chk finds the elite key
...
'''),
    # --- Write phase ---
    # writeDataBlock sends: hf iclass wrbl b X d DATA k KEY
    # hasKeyword("successful") must be true
    'hf iclass wrbl': (0, '''[+] Write block 6 successful
'''),
    # writePassword: calcnewkey computes XOR diversified key for password change
    'hf iclass calcnewkey': (0, '''[usb] pm3 --> hf iclass calcnewkey
...
'''),
    ...
    # Verify reads block 01 with the chk key + elite flag after password write
