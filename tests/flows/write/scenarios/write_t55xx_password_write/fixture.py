...
# T55XX password-protected tag restore flow:
#   READ: detect(original) x2 → read blocks → chk password → dump
#   WRITE: wipe p <pwd> → detect(wiped) x2 → restore → detect(restored) → read blocks (verify)
#
# lf t55xx detect uses SEQUENTIAL responses:
#   calls 1-2: original config (READ phase)
#   calls 3-4: wiped config (after wipe)
#   calls 5+:  restored config (after restore, for verify)
...
'''),
    'lf t55xx detect': [
        ...
        _DETECT_ORIGINAL,  # after restore (verify)
    ],
