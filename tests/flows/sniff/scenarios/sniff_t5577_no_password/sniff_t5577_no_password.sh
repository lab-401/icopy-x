#!/bin/bash
# Scenario: T5577 Sniff — data captured but no password (default writes, auto-finish)
# Flow: GOTO:4 → DOWN×4 → OK → M1 start → auto-stop via 125k_sniff_finished → result → save
# Ground truth: sniff_strings.txt regex "Default write\s+\|\s+([A-Fa-f0-9]{8})\s\|"
# NOTE: Fixture has 125k_sniff_finished marker — do NOT press M2 Finish.
PROJECT="${PROJECT:-/home/qx/icopy-x-reimpl}"
SCENARIO="sniff_t5577_no_password"
FLOW="sniff"
...
# Start — auto-stop via 125k_sniff_finished, no M2
send_key "M1"

if ! wait_for_ui_trigger "M2:Save" 30 "${raw_dir}" frame_idx; then
    dedup_screenshots "${raw_dir}" "${SCREENSHOTS_DIR}"
    report_fail "Auto-stop not reached (${DEDUP_COUNT} states)"
    cleanup_qemu; rm -rf "${raw_dir}"; exit 1
fi
