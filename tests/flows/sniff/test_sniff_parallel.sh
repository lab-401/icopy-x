#!/bin/bash
# Run ALL sniff scenarios in parallel using worker pools.
# Each scenario gets its own Xvfb display — fully isolated.
#
# Usage:
#   bash tests/flows/sniff/test_sniff_parallel.sh [JOBS]
#   bash tests/flows/sniff/test_sniff_parallel.sh --no-clean [JOBS]
#   bash tests/flows/sniff/test_sniff_parallel.sh --clean-flow-only [JOBS]
#
# Remote usage:
#   bash tests/flows/sniff/test_sniff_parallel.sh --remote USER@HOST [JOBS]
...
if [ "$1" = "--remote" ]; then
    ...
    echo "=== Running ${RJOBS} parallel sniff tests on ${REMOTE} ==="
    $SSH_CMD "$REMOTE" "cd ~/icopy-x-reimpl && TEST_TARGET=${TEST_TARGET:-current} bash tests/flows/sniff/test_sniff_parallel.sh --clean-flow-only ${RJOBS}" 2>&1 | tee /tmp/remote_sniff_log.txt

    echo "=== Pulling results ==="
    rm -rf "${SNIFF_RESULTS}"
    mkdir -p "${SNIFF_RESULTS}"
    eval $RSYNC_CMD "${REMOTE}:~/icopy-x-reimpl/tests/flows/_results/${TEST_TARGET:-current}/sniff/" "${SNIFF_RESULTS}/"
...
case "$CLEAN_MODE" in
    ...
    flow) rm -rf "${SNIFF_RESULTS}" ;;
    none) ;;
esac
mkdir -p "${SNIFF_RESULTS}"

# Collect scenario scripts
SCENARIO_BASE="${PROJECT}/tests/flows/sniff/scenarios"
SCRIPTS=()
for scenario_dir in "${SCENARIO_BASE}"/sniff_*/; do
    name="$(basename "$scenario_dir")"
    script="${scenario_dir}/${name}.sh"
    [ -f "$script" ] && SCRIPTS+=("$script")
done

echo "=== Sniff Flow: ${#SCRIPTS[@]} scenarios ==="
