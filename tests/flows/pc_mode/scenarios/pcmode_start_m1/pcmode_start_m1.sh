#!/bin/bash
# PC Mode — M1 triggers start from IDLE, buttons become disabled
# Real device: M1 from IDLE → STARTING (labels stay Start/Start, disabled)
PROJECT="${PROJECT:-/home/qx/icopy-x-reimpl}"
SCENARIO="pcmode_start_m1"
FLOW="pc_mode"
...
if ! wait_for_ui_trigger "M1_active:false" 10 "${raw_dir}" frame_idx; then
    dedup_screenshots "${raw_dir}" "${SCREENSHOTS_DIR}"
    report_fail "Buttons not disabled after M1 start"
    cleanup_qemu; rm -rf "${raw_dir}"; exit 1
fi
...
report_pass "M1 start verified: buttons disabled (${DEDUP_COUNT} states)"
cleanup_qemu
rm -rf "${raw_dir}"
